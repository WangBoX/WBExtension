...
+(BOOL) isEmpty:(id )data {
    ...
    if (!data || data == (id)kCFNull) {
        ...
        return false;
    }else if ([data isKindOfClass:[NSDictionary class]]){//数组空
    ...
    switch (code) {
        ...
        case error_business://业务异常
            if (autoShowError) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [NSObject hideWaitHUD];
                    [NSObject showHudTipStr:response.msgInfo];
                });
            }
            break;
        case error_reLogin:
            ...
            break;
        case error_buyMyself:{
            dispatch_async_main_safe(^(){
                [NSObject showWaitWithTip:@"你这么有艺术范玩自拍，会被人鄙视的！" withDuration:3.f finalAction:^{
        ...
        case error_PAY_VOUCH:{
            //出价接口已经单独处理【需参数：itemId，价格，故不能放在这里处理】
        }break;
        /* todo:全部多状态码处理
        case error_logout:
            [QJDataBase delUser];
        ...
        case error_invest:
            [self gotoQualifiedInvestorCertify];
            break;
        case error_risk:
            //未进行风险评估
            [self gotoRiskPingGu];
