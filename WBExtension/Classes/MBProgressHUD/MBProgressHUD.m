...
#pragma -添加paddingOffset
@property (nonatomic, strong) NSNumber *paddingOffset;
// Deprecated
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated hasGraceTime:(BOOL)graceTime {
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    hud.bezelView.style = MBProgressHUDBackgroundStyleSolidColor;
    hud.bezelView.backgroundColor = [UIColor blackColor];
...
- (void)updateConstraints {
    ...
    [topSpacer addConstraint:[NSLayoutConstraint constraintWithItem:topSpacer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:margin]];
    [bottomSpacer addConstraint:[NSLayoutConstraint constraintWithItem:bottomSpacer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:margin]];
...
#pragma -修改 MBDefaultPadding -> MBDefaultPadding + _paddingOffset
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MBDefaultPadding + [_paddingOffset floatValue] : 0.f;
